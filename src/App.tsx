...
function App() {
  ...
  return (
    <div className="App">
      <div className="flex justify-center items-center min-w-screen min-h-screen bg-gradient-to-r from-cyan-500 to-blue-500 ">
        <div className="p-5 rounded-xl lg:w-6/12 bg-white space-y-5">
          ...
          <ol className="items-center w-full space-y-4 sm:flex sm:space-x-8 sm:space-y-0 rtl:space-x-reverse p-3 bg-white border border-gray-200 rounded-lg shadow-sm">
            {steps.map((step, index) => (
              <li key={step.step} className={`flex items-center space-x-2.5 ${
                ...
                <span className={`flex items-center justify-center w-8 h-8 border rounded-full shrink-0 ${
                    currentStep === step.step ? 'border-blue-600' : 'border-gray-500' }`}>
                  {index + 1}
                </span>
                <span>
                ...
                if (field.type === 'textfield') {
                  return (
                    <div className="mb-5" key={index}>
                      <label htmlFor={field.label.toLowerCase()} className="block mb-2 text-sm font-medium text-gray-900 text-start">
                        {field.label}
                      </label>
                      <input
                        type="text"
                        ...
                        {field.options?.map((option, idx) => (
                          <div className="flex items-center me-4" key={idx}>
                            <input
                              ...
                              required={field.required}
                            />
                            <label htmlFor={option.value} className="ms-2 text-sm font-medium text-gray-900 cursor-pointer" >
                              {option.label}
                            </label>
                          </div>
                        ))}
                  ...
                  return (
                    <div className="mb-5" key={index}>
                      ...
                      <select id={field.label.toLowerCase()} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 block w-full p-2.5"
                        required={field.required}>
